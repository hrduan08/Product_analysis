...
12. [VoiceInsight 浏览器插件（P0）](#12-voiceinsight-浏览器插件p0)
...
### 7.1 用户搜索关键词同步
- 当前仅保留用户自助搜索任务；调度器由 `USER_SEARCH_CRON_*` 配置控制。
- 每次执行：
  - 调用 Provider 抓取最新内容（限制 `USER_SEARCH_FETCH_LIMIT`，默认 50）。
...
7. **搜索 API**：`YOUTUBE_API_KEY`、`REDDIT_PROVIDER`、`REDDIT_PUBLIC_JSON_USER_AGENT`、`REDDIT_USER_AGENT`、`REDDIT_BETA_WHITELIST`，以及 OAuth 备用凭证 `REDDIT_CLIENT_ID/SECRET/USERNAME/PASSWORD`。
8. **用户搜索任务**：`USER_SEARCH_CRON_ENABLED`、`USER_SEARCH_CRON_SCHEDULE`、`USER_SEARCH_CRON_RUN_ON_STARTUP`、`USER_SEARCH_CRON_TIMEZONE`、`USER_SEARCH_FETCH_LIMIT`、`USER_SEARCH_REDDIT_INTERVAL_MINUTES`、`USER_SEARCH_YOUTUBE_INTERVAL_MINUTES`、`USER_SEARCH_AUTO_SOON_MIN_SECONDS`、`USER_SEARCH_AUTO_SOON_MAX_SECONDS`、`USER_SEARCH_AUTO_JITTER_SECONDS`。
...
## 12. VoiceInsight 浏览器插件（P0）

### 12.1 目标与范围
- **目标**：在 YouTube 视频页通过 Side Panel 提供“看字幕、下字幕、复制字幕、切语言”四个核心能力，优先服务高频字幕处理场景。
- **本期优先级（P0）**：
  1. 自动提取字幕并按时间戳展示（对应竞品功能 1）。
  2. 下载字幕文件（TXT/SRT/VTT）（对应竞品功能 5）。
  3. 复制字幕（对应竞品功能 6）。
  4. 设置：仅插件界面语言设置（对应竞品功能 7，范围收敛）。
- **明确不在本期**：翻译与双语展示、时间戳显隐、shorten/extend 时间间隔调整（以上归入 P1）。

### 12.2 使用前提
- 浏览器：Chrome / Edge（Manifest V3）。
- 页面：仅支持 `youtube.com/watch` 与 `youtu.be/*` 视频页。
- 字幕前提：视频需存在可提取字幕轨道；无轨道时返回失败提示，不做 OCR/ASR 兜底。

### 12.3 功能需求明细（P0）

#### 12.3.1 F1 自动提取字幕并按时间戳展示
- **触发**：用户在 YouTube 视频页点击插件图标，打开 Side Panel。
- **系统行为**：
  1. 自动识别当前活动视频页与 `videoId`。
  2. 自动读取可用字幕轨道并提取字幕片段。
  3. 将字幕按时间顺序渲染到 Side Panel 列表（默认显示时间戳）。
- **展示规则**：
  - 每条字幕至少包含：`timestamp`、`text`。
  - 时间戳格式统一为 `mm:ss`（超过 1 小时可扩展为 `hh:mm:ss`）。
  - 列表按时间升序展示，支持滚动浏览。
- **状态要求**：
  - `idle`：未开始。
  - `loading`：提取中，展示进度或状态文案。
  - `success`：展示字幕列表与操作区。
  - `error`：展示错误文案，可重试。
- **错误文案基线**：
  - 非 YouTube 视频页：`请先打开一个 YouTube 视频页面。`
  - 无可提取字幕：`该视频无字幕可下载。`
  - 其他失败：`操作失败，请稍后重试。`

#### 12.3.2 F5 下载字幕文件（TXT / SRT / VTT）
- **触发**：用户点击 `Download` 按钮。
- **格式选择**：提供 `TXT`、`SRT`、`VTT` 三种格式切换，默认值可由上次选择记忆。
- **导出规则**：
  - TXT：每行输出 `[{timestamp}] {text}`。
  - SRT：输出序号、时间区间（`hh:mm:ss,SSS --> hh:mm:ss,SSS`）、文本。
  - VTT：输出 `WEBVTT` 头与时间区间（`hh:mm:ss.SSS --> hh:mm:ss.SSS`）、文本。
- **文件命名**：`VoiceInsight-{VideoTitle}.{ext}`（标题非法字符需清洗）。
- **成功后行为**：显示“下载成功”状态，不主动改变字幕内容与顺序。

#### 12.3.3 F6 复制字幕
- **触发**：用户点击 `Copy` 按钮。
- **复制内容**：
  - 按当前展示顺序复制全部字幕。
  - 默认包含时间戳与文本，行与行之间用换行分隔。
- **交互反馈**：
  - 成功提示：`已复制字幕内容。`
  - 失败提示：`复制失败，请稍后重试。`

#### 12.3.4 F7 设置（仅界面语言）
- **设置项**：插件界面语言（`中文` / `English`）。
- **生效方式**：切换后立即生效，无需刷新页面。
- **持久化**：写入 `chrome.storage.local`，下次打开继续使用上次语言。
- **覆盖范围**：按钮文案、状态文案、错误文案、辅助说明文案。

### 12.4 数据与存储约束（P0）
- **字幕片段模型（逻辑）**：
  - `timestamp: string`
  - `startMs: number`（可选，供导出 SRT/VTT）
  - `endMs: number`（可选，供导出 SRT/VTT）
  - `text: string`
- **本地存储键（建议）**：
  - `voiceInsightLocaleOverride`：界面语言覆盖值。
  - `voiceInsightSubtitleFormat`：上次下载格式。

### 12.5 交互流程（P0）
```mermaid
flowchart TD
  A[用户在 YouTube 视频页点击插件图标] --> B[打开 Side Panel]
  B --> C[自动提取字幕]
  C -->|成功| D[按时间戳展示字幕]
  C -->|失败| E[展示错误文案]
  D --> F[Download: 导出 TXT/SRT/VTT]
  D --> G[Copy: 复制字幕]
  D --> H[Settings: 切换界面语言]
```

### 12.6 验收标准（P0）
1. 在有字幕轨道的视频页，打开插件后可自动展示时间戳字幕列表。
2. 在无字幕轨道的视频页，明确提示“该视频无字幕可下载。”且不出现崩溃。
3. 下载支持 TXT/SRT/VTT 三种格式，文件可被常见文本工具正常打开。
4. 复制功能可一次性复制当前字幕列表内容，粘贴内容与页面一致。
5. 语言设置在重开插件后仍保持上次选择。
6. 全流程仅修改插件侧能力，不依赖 VoiceInsight Web 后端与数据库改动。
