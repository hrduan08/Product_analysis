...
export function SearchConfigPage() {
    ...
    useEffect(() => {
        ...
        const load = async () => {
            ...
            try {
                ...
                setTimezoneDraft(response.config.timezone);
    ...
    const applyPatch = async (payload, requestPayload) => {
        ...
        try {
            ...
            setTimezoneDraft(previous.timezone);
    ...
    const handleTimezoneBlur = async () => {
        if (!config)
            return;
        const value = timezoneDraft.trim() || config.timezone;
        if (value === config.timezone) {
            setTimezoneDraft(config.timezone);
            return;
        }
        await applyPatch({ timezone: value }, { timezone: value });
    };
