...
async function runDownloadTask(tabInfo, task) {
  ...
  try {
    ...
    const timeline = Array.isArray(domTranscript?.timeline)
      ? domTranscript.timeline
      : [];
...
async function fetchTranscriptFromDomWithRetry(tabId, signal, task) {
  ...
  for (let i = 0; i < delays.length; i += 1) {
    ...
    try {
      ...
      return {
        transcript,
        timeline: normalizeTimelineEntries(result?.timeline),
...
function buildCaptionText({ text, timeline, title, url, language, auto }) {
  const lines = [];
  ...
  if (normalizedTimeline.length > 0) {
    normalizedTimeline.forEach((entry) => {
...
function normalizeTimelineEntries(entries) {
  ...
  for (const entry of entries) {
    ...
    normalized.push({ timestamp, text });
  }
  return normalized;
}
