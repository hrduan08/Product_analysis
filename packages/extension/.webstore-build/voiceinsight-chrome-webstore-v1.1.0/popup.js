const DEBUG_BUILD_LABEL = "v1.1.0-debug";
...
const ACTIVE_TRANSCRIPT_CHANNEL_MODE = TRANSCRIPT_CHANNEL_MODE.ytPanelTranscriptOnly;
...
function resolveTranscriptLanguageLabel() {
  ...
  if (state.transcriptIsAutoGenerated) {
    resolved = `${resolved} (auto-generated)`;
  }
  return resolved;
}
...
async function refreshTranscript(force = false, options = {}) {
  ...
  const task = (async () => {
    ...
    try {
      ...
      if (!isYtPanelTranscriptOnlyMode()) {
...
function assessTimestampAnchorQuality(lines, anchors) {
  ...
  const expectedMinAnchors = Math.max(
    5,
    Math.min(140, Math.floor(lineMaxSec / TIMESTAMP_ALIGN_EXPECTED_ANCHOR_SPAN_SEC))
  );
  ...
  if (coverage < TIMESTAMP_ALIGN_QUALITY_MIN_COVERAGE) {
    return {
      ok: false,
...
function shouldAlignDisplayTimestamps(transcript) {
  ...
  return source.includes("youtube-transcript-api") || source.includes("yt-dlp");
}
...
function rebuildDisplayLines() {
  ...
  if (state.timeModeEnabled) {
    // Keep the original transcript cadence in time mode.
    state.lines = state.rawLines.slice();
    return;
  }
  state.lines = buildTextOnlyTimeline(state.rawLines);
}
    ...
    chrome.scripting.executeScript(
      {
        target: { tabId },
        world: "MAIN",
        args: [{ allowOpenPanel, timeoutMs, retryHint, minTimeoutMs }],
        func: async (input) => {
                ...
                if (openResult?.surface && typeof openResult.surface === "object") {
                  debug.openSurface = {
		            ...
		            const maybeStartInnertubeProbe = () => {
		              if (settled || debug.innertubeProbeTried) return;
...
async function fetchTranscriptFromTimedtextList(tabInfo) {
  ...
  const tracks = await fetchTimedTextTrackList(videoId, tabInfo.tabId);
  ...
  const preferredLanguage = state.locale === "zh" ? "zh" : "en";
  const ordered = orderTimedTextTracks(tracks, preferredLanguage);
  for (const track of ordered) {
    const baseUrl = buildTimedTextTrackUrl(videoId, track);
...
function orderTimedTextTracks(tracks, preferredLanguage) {
  const normalized = [];
  const seen = new Set();
  tracks.forEach((track) => {
    ...
    const key = `${item.languageCode}|${item.name}|${item.isAutoGenerated ? "asr" : "manual"}`;
    if (seen.has(key)) return;
    seen.add(key);
  ...
  const preferredMatches = preferred
    ? normalized.filter((item) =>
        item.languageCode.toLowerCase().startsWith(preferred)
      )
    : [];
  ...
  const manualOthers = normalized.filter(
    (item) =>
      ...
      !manualPreferred.some((pick) => sameTrack(pick, item))
  );
  const autoOthers = normalized.filter(
    (item) =>
      item.isAutoGenerated &&
      !autoPreferred.some((pick) => sameTrack(pick, item))
  );
...
function sameTrack(a, b) {
  return (
    ...
    a.name === b.name &&
    a.isAutoGenerated === b.isAutoGenerated
  );
}

async function fetchTimedTextTrackList(videoId, tabId) {
  const listUrl = `https://www.youtube.com/api/timedtext?type=list&v=${encodeURIComponent(
    videoId
  )}`;
  const payloadResult = await fetchCaptionPayloadDetailed(listUrl, tabId);
  const payload = String(payloadResult?.payload || "");
  const tracks = parseTimedTextTrackList(payload);
  addDiagStep("TIMEDTEXT_LIST_FETCH", payloadResult?.ok ? "ok" : "retry", {
...
function parseTimedTextTrackList(xmlText) {
  if (!xmlText || !String(xmlText).trim().startsWith("<")) return [];
  const tracks = [];
  ...
  while ((match = regex.exec(String(xmlText)))) {
    const attrs = match[1] || "";
    const languageCode =
      decodeEntities(extractXmlAttribute(attrs, "lang_code")) ||
      decodeEntities(extractXmlAttribute(attrs, "lang")) ||
      "";
    if (!languageCode) continue;

    const kind = decodeEntities(extractXmlAttribute(attrs, "kind"));
    const name = decodeEntities(extractXmlAttribute(attrs, "name"));
    tracks.push({
      languageCode,
      name,
